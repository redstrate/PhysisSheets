...
let exh = game_data.read_excel_sheet_header("MiniGameTurnBreakEnemy").unwrap();let exd = game_data.read_excel_sheet("MiniGameTurnBreakEnemy", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> MiniGameTurnBreakEnemyRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
MiniGameTurnBreakEnemyRow { columns: row.columns.clone() }
}
}
