...
pub struct Frontline01 {
exd: EXD,
exh: EXH,
}
impl Frontline01 {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("Frontline01").unwrap();let exd = game_data.read_excel_sheet("Frontline01", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> Frontline01Row {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
Frontline01Row { columns: row.columns.clone() }
}
}
