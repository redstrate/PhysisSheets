...
pub struct WKSFortunePatternUIPlace {
exd: EXD,
exh: EXH,
}
impl WKSFortunePatternUIPlace {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("WKSFortunePatternUIPlace").unwrap();let exd = game_data.read_excel_sheet("WKSFortunePatternUIPlace", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSFortunePatternUIPlaceRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSFortunePatternUIPlaceRow { columns: row.columns.clone() }
}
}
pub struct WKSFortunePatternUIPlaceRow {
columns: Vec<ColumnData>,
}
impl WKSFortunePatternUIPlaceRow {
