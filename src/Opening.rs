...
let exh = game_data.read_excel_sheet_header("Opening").unwrap();let exd = game_data.read_excel_sheet("Opening", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> OpeningRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
OpeningRow { columns: row.columns.clone() }
}
}
