#![allow(warnings)]
/// This file is auto-generated! It is generated from schema from https://github.com/xivdev/EXDSchema.
use physis::{gamedata::GameData, exd::{EXD, ColumnData, ExcelRowKind}, exh::EXH, common::Language};
pub struct EventItem {
exd: EXD,
exh: EXH,
}
impl EventItem {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("EventItem").unwrap();let exd = game_data.read_excel_sheet("EventItem", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> EventItemRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
EventItemRow { columns: row.columns.clone() }
}
}
pub struct EventItemRow {
columns: Vec<ColumnData>,
}
impl EventItemRow {
pub fn Singular(&self) -> &ColumnData {
&self.columns[0]
}
pub fn Plural(&self) -> &ColumnData {
&self.columns[1]
}
pub fn Name(&self) -> &ColumnData {
&self.columns[2]
}
pub fn Adjective(&self) -> &ColumnData {
&self.columns[3]
}
pub fn PossessivePronoun(&self) -> &ColumnData {
&self.columns[4]
}
pub fn StartsWithVowel(&self) -> &ColumnData {
&self.columns[5]
}
pub fn Unknown0(&self) -> &ColumnData {
&self.columns[6]
}
pub fn Pronoun(&self) -> &ColumnData {
&self.columns[7]
}
pub fn Article(&self) -> &ColumnData {
&self.columns[8]
}
pub fn Unknown1(&self) -> &ColumnData {
&self.columns[9]
}
pub fn Quest(&self) -> &ColumnData {
&self.columns[10]
}
pub fn Icon(&self) -> &ColumnData {
&self.columns[11]
}
pub fn Action(&self) -> &ColumnData {
&self.columns[12]
}
pub fn StackSize(&self) -> &ColumnData {
&self.columns[13]
}
pub fn Category(&self) -> &ColumnData {
&self.columns[14]
}
pub fn CastTime(&self) -> &ColumnData {
&self.columns[15]
}
pub fn CastTimeline(&self) -> &ColumnData {
&self.columns[16]
}
pub fn Timeline(&self) -> &ColumnData {
&self.columns[17]
}
}
