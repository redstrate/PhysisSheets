...
pub struct WKSDevGradeBGSet {
exd: EXD,
exh: EXH,
}
impl WKSDevGradeBGSet {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("WKSDevGradeBGSet").unwrap();let exd = game_data.read_excel_sheet("WKSDevGradeBGSet", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSDevGradeBGSetRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSDevGradeBGSetRow { columns: row.columns.clone() }
}
}
