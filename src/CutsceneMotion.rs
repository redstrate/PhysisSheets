...
let exh = game_data.read_excel_sheet_header("CutsceneMotion").unwrap();let exd = game_data.read_excel_sheet("CutsceneMotion", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> CutsceneMotionRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
CutsceneMotionRow { columns: row.columns.clone() }
}
}
