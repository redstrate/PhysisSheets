...
let exh = game_data.read_excel_sheet_header("TreasureHuntRank")?;let exd = game_data.read_excel_sheet("TreasureHuntRank", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
...
Some(TreasureHuntRankRow { columns })
}
/// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
pub fn get_row(&self, row_id: u32) -> Option<TreasureHuntRankRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<TreasureHuntRankRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
