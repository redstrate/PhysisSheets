...
let exh = game_data.read_excel_sheet_header("CharaCardDesignPreset")?;let exd = game_data.read_excel_sheet("CharaCardDesignPreset", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
...
Some(CharaCardDesignPresetRow { columns })
}
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<CharaCardDesignPresetRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
