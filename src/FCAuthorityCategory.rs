...
fn read_row(&self, row: &ExcelSingleRow) -> Option<FCAuthorityCategoryRow> {
...
pub fn get_row(&self, row_id: u32) -> Option<FCAuthorityCategoryRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<FCAuthorityCategoryRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
