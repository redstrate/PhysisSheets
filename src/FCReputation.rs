...
let exh = game_data.read_excel_sheet_header("FCReputation")?;let exd = game_data.read_excel_sheet("FCReputation", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
...
Some(FCReputationRow { columns })
}
/// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
pub fn get_row(&self, row_id: u32) -> Option<FCReputationRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<FCReputationRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
