...
let exh = game_data.read_excel_sheet_header("ShellFixedFromCommand").unwrap();let exd = game_data.read_excel_sheet("ShellFixedFromCommand", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ShellFixedFromCommandRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ShellFixedFromCommandRow { columns: row.columns.clone() }
}
}
