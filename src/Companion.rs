...
let exh = game_data.read_excel_sheet_header("Companion").unwrap();let exd = game_data.read_excel_sheet("Companion", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> CompanionRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
CompanionRow { columns: row.columns.clone() }
}
}
