...
let exh = game_data.read_excel_sheet_header("FittingShop").unwrap();let exd = game_data.read_excel_sheet("FittingShop", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> FittingShopRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
FittingShopRow { columns: row.columns.clone() }
}
}
