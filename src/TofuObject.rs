...
let exh = game_data.read_excel_sheet_header("TofuObject").unwrap();let exd = game_data.read_excel_sheet("TofuObject", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> TofuObjectRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
TofuObjectRow { columns: row.columns.clone() }
}
}
