...
let exh = game_data.read_excel_sheet_header("InstanceContent").unwrap();let exd = game_data.read_excel_sheet("InstanceContent", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> InstanceContentRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
InstanceContentRow { columns: row.columns.clone() }
}
}
