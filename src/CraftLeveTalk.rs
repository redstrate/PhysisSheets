...
let exh = game_data.read_excel_sheet_header("CraftLeveTalk").unwrap();let exd = game_data.read_excel_sheet("CraftLeveTalk", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> CraftLeveTalkRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
CraftLeveTalkRow { columns: row.columns.clone() }
}
}
