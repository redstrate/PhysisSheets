...
let exh = game_data.read_excel_sheet_header("Frontline").unwrap();let exd = game_data.read_excel_sheet("Frontline", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> FrontlineRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
FrontlineRow { columns: row.columns.clone() }
}
}
