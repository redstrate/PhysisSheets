...
let exh = game_data.read_excel_sheet_header("GoldSaucerArcadeMachine").unwrap();let exd = game_data.read_excel_sheet("GoldSaucerArcadeMachine", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> GoldSaucerArcadeMachineRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
GoldSaucerArcadeMachineRow { columns: row.columns.clone() }
}
}
...
pub fn Excellent(&self) -> &ColumnData {
&self.columns[19]
}
...
pub fn FailImage(&self) -> &ColumnData {
&self.columns[28]
}
