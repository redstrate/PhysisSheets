...
pub struct Attributive {
exd: EXD,
exh: EXH,
}
impl Attributive {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("Attributive").unwrap();let exd = game_data.read_excel_sheet("Attributive", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> AttributiveRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
AttributiveRow { columns: row.columns.clone() }
}
}
