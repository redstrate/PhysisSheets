...
let exh = game_data.read_excel_sheet_header("ENpcResident").unwrap();let exd = game_data.read_excel_sheet("ENpcResident", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ENpcResidentRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ENpcResidentRow { columns: row.columns.clone() }
}
}
