...
let exh = game_data.read_excel_sheet_header("ClassJobCategory").unwrap();let exd = game_data.read_excel_sheet("ClassJobCategory", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ClassJobCategoryRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ClassJobCategoryRow { columns: row.columns.clone() }
}
}
