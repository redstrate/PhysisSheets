...
let exh = game_data.read_excel_sheet_header("ContentRoulette").unwrap();let exd = game_data.read_excel_sheet("ContentRoulette", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ContentRouletteRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ContentRouletteRow { columns: row.columns.clone() }
}
}
