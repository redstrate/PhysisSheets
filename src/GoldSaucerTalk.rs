...
pub struct GoldSaucerTalk {
exd: EXD,
exh: EXH,
}
impl GoldSaucerTalk {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("GoldSaucerTalk").unwrap();let exd = game_data.read_excel_sheet("GoldSaucerTalk", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> GoldSaucerTalkRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
GoldSaucerTalkRow { columns: row.columns.clone() }
}
}
