...
pub struct JigsawTimeBonusSheet {
exd: EXD,
exh: EXH,
}
impl JigsawTimeBonusSheet {
pub fn read_from(game_data: &mut GameData, language: Language) -> Option<Self> {
let exh = game_data.read_excel_sheet_header("JigsawTimeBonus")?;let exd = game_data.read_excel_sheet("JigsawTimeBonus", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
...
Some(JigsawTimeBonusRow { columns })
}
/// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
pub fn get_row(&self, row_id: u32) -> Option<JigsawTimeBonusRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<JigsawTimeBonusRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
