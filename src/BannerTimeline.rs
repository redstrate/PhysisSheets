...
let exh = game_data.read_excel_sheet_header("BannerTimeline").unwrap();let exd = game_data.read_excel_sheet("BannerTimeline", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> BannerTimelineRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
BannerTimelineRow { columns: row.columns.clone() }
}
}
