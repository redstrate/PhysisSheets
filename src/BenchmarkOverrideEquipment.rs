...
let exh = game_data.read_excel_sheet_header("BenchmarkOverrideEquipment").unwrap();let exd = game_data.read_excel_sheet("BenchmarkOverrideEquipment", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> BenchmarkOverrideEquipmentRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
BenchmarkOverrideEquipmentRow { columns: row.columns.clone() }
}
}
