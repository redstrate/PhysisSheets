...
let exh = game_data.read_excel_sheet_header("CompanionTransient").unwrap();let exd = game_data.read_excel_sheet("CompanionTransient", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> CompanionTransientRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
CompanionTransientRow { columns: row.columns.clone() }
}
}
