...
let exh = game_data.read_excel_sheet_header("HousingPreset").unwrap();let exd = game_data.read_excel_sheet("HousingPreset", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> HousingPresetRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
HousingPresetRow { columns: row.columns.clone() }
}
}
