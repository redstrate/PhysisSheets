...
pub struct WKSCosmoToolClass {
exd: EXD,
exh: EXH,
}
impl WKSCosmoToolClass {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("WKSCosmoToolClass").unwrap();let exd = game_data.read_excel_sheet("WKSCosmoToolClass", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSCosmoToolClassRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSCosmoToolClassRow { columns: row.columns.clone() }
}
}
