...
let exh = game_data.read_excel_sheet_header("Item").unwrap();let exd = game_data.read_excel_sheet("Item", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ItemRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ItemRow { columns: row.columns.clone() }
}
}
pub struct ItemRow {
