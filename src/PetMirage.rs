...
let exh = game_data.read_excel_sheet_header("PetMirage").unwrap();let exd = game_data.read_excel_sheet("PetMirage", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> PetMirageRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
PetMirageRow { columns: row.columns.clone() }
}
}
