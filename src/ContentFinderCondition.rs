...
let exh = game_data.read_excel_sheet_header("ContentFinderCondition").unwrap();let exd = game_data.read_excel_sheet("ContentFinderCondition", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ContentFinderConditionRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ContentFinderConditionRow { columns: row.columns.clone() }
}
}
