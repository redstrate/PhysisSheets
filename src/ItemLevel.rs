...
let exh = game_data.read_excel_sheet_header("ItemLevel").unwrap();let exd = game_data.read_excel_sheet("ItemLevel", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ItemLevelRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ItemLevelRow { columns: row.columns.clone() }
}
}
