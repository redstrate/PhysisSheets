...
pub struct EmjVoiceNpc {
exd: EXD,
exh: EXH,
}
impl EmjVoiceNpc {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("EmjVoiceNpc").unwrap();let exd = game_data.read_excel_sheet("EmjVoiceNpc", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> EmjVoiceNpcRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
EmjVoiceNpcRow { columns: row.columns.clone() }
}
}
