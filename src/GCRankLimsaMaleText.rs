...
let exh = game_data.read_excel_sheet_header("GCRankLimsaMaleText")?;let exd = game_data.read_excel_sheet("GCRankLimsaMaleText", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
fn read_row(&self, row: &ExcelSingleRow) -> Option<GCRankLimsaMaleTextRow> {
let column_defs = &self.exh.column_definitions;
let mut zipped: Vec<_> = row.columns.clone().into_iter().zip(column_defs).collect();
zipped.sort_by(|(_, a_col), (_, b_col)| a_col.offset.cmp(&b_col.offset));
let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition> ) = zipped.into_iter().unzip();
Some(GCRankLimsaMaleTextRow { columns })
}
/// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
pub fn get_row(&self, row_id: u32) -> Option<GCRankLimsaMaleTextRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<GCRankLimsaMaleTextRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
