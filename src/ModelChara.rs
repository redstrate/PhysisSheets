...
let exh = game_data.read_excel_sheet_header("ModelChara").unwrap();let exd = game_data.read_excel_sheet("ModelChara", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ModelCharaRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ModelCharaRow { columns: row.columns.clone() }
}
}
...
pub fn PapVariation(&self) -> &ColumnData {
&self.columns[16]
}
