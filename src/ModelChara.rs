...
let exh = game_data.read_excel_sheet_header("ModelChara")?;let exd = game_data.read_excel_sheet("ModelChara", &exh, language, 0)?;Some(Self {
exh,
exd,
})
}
...
Some(ModelCharaRow { columns })
}
/// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
pub fn get_row(&self, row_id: u32) -> Option<ModelCharaRow> {
...
pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<ModelCharaRow> {
let Some(row) = &self.exd.get_row(row_id) else { return None; };
let row = match row {
