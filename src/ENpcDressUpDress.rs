...
let exh = game_data.read_excel_sheet_header("ENpcDressUpDress").unwrap();let exd = game_data.read_excel_sheet("ENpcDressUpDress", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ENpcDressUpDressRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ENpcDressUpDressRow { columns: row.columns.clone() }
}
}
