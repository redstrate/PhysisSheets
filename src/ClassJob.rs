...
let exh = game_data.read_excel_sheet_header("ClassJob").unwrap();let exd = game_data.read_excel_sheet("ClassJob", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> ClassJobRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
ClassJobRow { columns: row.columns.clone() }
}
}
