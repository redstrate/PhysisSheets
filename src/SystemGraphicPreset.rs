...
pub struct SystemGraphicPreset {
exd: EXD,
exh: EXH,
}
impl SystemGraphicPreset {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("SystemGraphicPreset").unwrap();let exd = game_data.read_excel_sheet("SystemGraphicPreset", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> SystemGraphicPresetRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
SystemGraphicPresetRow { columns: row.columns.clone() }
}
}
