...
pub struct TerritoryIntendedUse {
exd: EXD,
exh: EXH,
}
impl TerritoryIntendedUse {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("TerritoryIntendedUse").unwrap();let exd = game_data.read_excel_sheet("TerritoryIntendedUse", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> TerritoryIntendedUseRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
TerritoryIntendedUseRow { columns: row.columns.clone() }
}
}
