...
let exh = game_data.read_excel_sheet_header("WKSMissionUnit").unwrap();let exd = game_data.read_excel_sheet("WKSMissionUnit", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSMissionUnitRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSMissionUnitRow { columns: row.columns.clone() }
}
}
