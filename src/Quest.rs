...
let exh = game_data.read_excel_sheet_header("Quest").unwrap();let exd = game_data.read_excel_sheet("Quest", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> QuestRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
QuestRow { columns: row.columns.clone() }
}
}
pub struct QuestRow {
