...
pub struct WKSCosmoToolDataAmount {
exd: EXD,
exh: EXH,
}
impl WKSCosmoToolDataAmount {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("WKSCosmoToolDataAmount").unwrap();let exd = game_data.read_excel_sheet("WKSCosmoToolDataAmount", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSCosmoToolDataAmountRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSCosmoToolDataAmountRow { columns: row.columns.clone() }
}
}
