...
pub struct QuestSubCommand {
exd: EXD,
exh: EXH,
}
impl QuestSubCommand {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("QuestSubCommand").unwrap();let exd = game_data.read_excel_sheet("QuestSubCommand", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> QuestSubCommandRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
QuestSubCommandRow { columns: row.columns.clone() }
}
}
