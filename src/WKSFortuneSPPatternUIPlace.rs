...
let exh = game_data.read_excel_sheet_header("WKSFortuneSPPatternUIPlace").unwrap();let exd = game_data.read_excel_sheet("WKSFortuneSPPatternUIPlace", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> WKSFortuneSPPatternUIPlaceRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
WKSFortuneSPPatternUIPlaceRow { columns: row.columns.clone() }
}
}
