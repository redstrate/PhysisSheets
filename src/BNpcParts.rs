...
let exh = game_data.read_excel_sheet_header("BNpcParts").unwrap();let exd = game_data.read_excel_sheet("BNpcParts", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> BNpcPartsRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
BNpcPartsRow { columns: row.columns.clone() }
}
}
