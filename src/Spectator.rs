...
let exh = game_data.read_excel_sheet_header("Spectator").unwrap();let exd = game_data.read_excel_sheet("Spectator", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> SpectatorRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
SpectatorRow { columns: row.columns.clone() }
}
}
