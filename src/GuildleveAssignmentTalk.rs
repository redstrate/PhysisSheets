...
let exh = game_data.read_excel_sheet_header("GuildleveAssignmentTalk").unwrap();let exd = game_data.read_excel_sheet("GuildleveAssignmentTalk", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> GuildleveAssignmentTalkRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
GuildleveAssignmentTalkRow { columns: row.columns.clone() }
}
}
