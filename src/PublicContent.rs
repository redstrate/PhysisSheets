#![allow(warnings)]
/// This file is auto-generated! It is generated from schema from https://github.com/xivdev/EXDSchema.
use physis::{gamedata::GameData, exd::{EXD, ColumnData, ExcelRowKind}, exh::EXH, common::Language};
pub struct PublicContent {
exd: EXD,
exh: EXH,
}
impl PublicContent {
pub fn read_from(game_data: &mut GameData, language: Language) -> Self {
let exh = game_data.read_excel_sheet_header("PublicContent").unwrap();let exd = game_data.read_excel_sheet("PublicContent", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> PublicContentRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
PublicContentRow { columns: row.columns.clone() }
}
}
pub struct PublicContentRow {
columns: Vec<ColumnData>,
}
impl PublicContentRow {
pub fn Name(&self) -> &ColumnData {
&self.columns[0]
}
pub fn MapIcon(&self) -> &ColumnData {
&self.columns[1]
}
pub fn TextDataStart(&self) -> &ColumnData {
&self.columns[2]
}
pub fn TextDataEnd(&self) -> &ColumnData {
&self.columns[3]
}
pub fn StartCutscene(&self) -> &ColumnData {
&self.columns[4]
}
pub fn LGBEventRange(&self) -> &ColumnData {
&self.columns[5]
}
pub fn LGBPopRange(&self) -> &ColumnData {
&self.columns[6]
}
pub fn EndCutscene(&self) -> &ColumnData {
&self.columns[7]
}
pub fn TimeLimit(&self) -> &ColumnData {
&self.columns[8]
}
pub fn ContentFinderCondition(&self) -> &ColumnData {
&self.columns[9]
}
pub fn AdditionalData(&self) -> &ColumnData {
&self.columns[10]
}
pub fn Unknown0(&self) -> &ColumnData {
&self.columns[11]
}
pub fn Unknown1(&self) -> &ColumnData {
&self.columns[12]
}
pub fn Unknown2(&self) -> &ColumnData {
&self.columns[13]
}
pub fn Unknown3(&self) -> &ColumnData {
&self.columns[14]
}
pub fn Unknown5(&self) -> &ColumnData {
&self.columns[15]
}
pub fn Type(&self) -> &ColumnData {
&self.columns[16]
}
pub fn Unknown4(&self) -> &ColumnData {
&self.columns[17]
}
}
