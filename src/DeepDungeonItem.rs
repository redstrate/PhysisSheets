...
let exh = game_data.read_excel_sheet_header("DeepDungeonItem").unwrap();let exd = game_data.read_excel_sheet("DeepDungeonItem", &exh, language, 0).unwrap();Self {
exh,
exd,
}
}
pub fn get_row(&self, id: u32) -> DeepDungeonItemRow {let ExcelRowKind::SingleRow(row) = &self.exd.get_row(id).unwrap() else { panic!("Expected a single row!"); };
DeepDungeonItemRow { columns: row.columns.clone() }
}
}
